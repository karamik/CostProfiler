...
func main() {
	...
	case "java":
		// Если запрошен сигнал, отправляем SIGUSR2 перед чтением perf-map
		if javaSignal {
	...
	defer func() {
		// Закрываем все fd, сохранённые в мапе
		var key, fd uint64
	...
	// Дополнительная рекомендация по inlined
	hasInlined := false
	for _, sym := range symbols {
		if sym.Inlined {
			hasInlined = true
