...
func printTable(metrics []FunctionMetric, config CostConfig) {
	...
	for _, m := range metrics {
		if m.CallCount == 0 {
			// Для inlined-функций покажем серую строку с нулями
			table.Rich([]string{
...
func printCSV(metrics []FunctionMetric, config CostConfig) {
	...
	for _, m := range metrics {
		if m.CallCount == 0 {
			continue // пропускаем inlined в CSV
		}
		avgMs := float64(m.TotalNS) / float64(m.CallCount) / 1_000_000
...
func main() {
	...
	flag.StringVar(&webPort, "web", "", "Запустить веб-интерфейс на указанном порту (например, :8080)")
	flag.Parse()
	...
	// Открытие исполняемого файла для uprobe
	...
	// Установка uprobe для не-inlined функций
	...
	// Для всех потоков процесса открываем perf_event для сбора CPU cycles
	...
	// Запуск веб-сервера (если указан флаг)
	if webPort != "" {
	...
	// добавляем реальные вызовы из eBPF
	...
	// добавляем inlined‑функции (без вызовов) для информации
	for _, sym := range symbols {
		if sym.Inlined {
			metricList = append(metricList, FunctionMetric{
	...
	// Сохраняем в глобальную переменную для веб-интерфейса
	metricsMutex.Lock()
	...
	// Если веб-сервер запущен – ждём окончания работы (иначе сразу выходим)
	if webPort != "" {
