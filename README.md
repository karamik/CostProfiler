...
## ⚡ Profile-Guided Optimization (PGO) — новая возможность
...
2. **Пересобираем приложение** с этим профилем:
   ...
   ```
3. **Результат** — бинарник, оптимизированный под реальное поведение (ускорение hot-путей до 15-20% без изменения кода).

### Пример с демо

```bash
cd CostProfiler
make demo              # автоматически создаст profile.pprof и покажет инструкцию
make build-pgo         # сборка cost-profiler с PGO-оптимизацией
```
...
| `--export-pgo` | "" | Экспортировать профиль в файл для `go build -pgo` |
...
```bash
$ sudo cost-profiler --pid 27182 --binary ./logistics-service --duration 15 --export-pgo profile.pprof
...
✅ PGO профиль экспортирован в profile.pprof
   Используйте для оптимизации: go build -pgo=profile.pprof -o optimized-binary
```
  ...
  cost-check:
    ...
    steps:
      ...
      - name: Optimize with PGO
        run: |
          go build -pgo=profile.pprof -o service-optimized main.go
...
| **Дополнительный выигрыш от PGO** | **+10-20% производительности без изменения кода** |
...
**Q: Как использовать PGO?**  
A: Соберите профиль с `--export-pgo profile.pprof`, затем пересоберите приложение: `go build -pgo=profile.pprof -o optimized-binary`.

**Q: Насколько эффективен PGO?**  
A: В типичных Go-сервисах ускорение hot-путей составляет 10-20% без изменения исходного кода.
