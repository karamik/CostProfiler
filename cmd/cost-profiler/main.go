...
import (
	...
	// Сгенерированный пакет (после go generate)
...
func findSymbols(binaryPath string, filter string) ([]Symbol, error) {
	...
	for _, sym := range symbols {
		// Только функции, не runtime, не внутренние
...
func main() {
	...
	for _, sym := range symbols {
		// Пропускаем слишком короткие функции (вероятно, инлайн)
		if len(sym.Name) < 5 {
			continue
		}
	...
	go func() {
		for range ticker.C {
			...
			// Чтение счётчиков вызовов
			callIter := objs.FunctionCalls.Iterate()
			for callIter.Next(&key, &calls) {
				if cyclesErr := objs.FunctionCycles.Lookup(&key, &cycles); cyclesErr != nil {
					continue
				}
				if calls > 0 && cycles > 0 {
	...
	for key, m := range metrics {
		// Поиск имени функции по адресу (упрощённо, в реальности нужно маппинг)
	...
	// Сортировка по затратам (пузырьком для простоты)
