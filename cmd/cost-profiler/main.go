...
var (
	lastMetrics   []FunctionMetric
	metricsMutex  sync.RWMutex
)

// Prometheus метрики
var (
	functionCallsMetric = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cost_profiler_function_calls_total",
			Help: "Total number of calls per function",
		},
		[]string{"function"},
	)
	functionCyclesMetric = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cost_profiler_function_cycles_total",
			Help: "Total CPU cycles (nanoseconds) per function",
		},
		[]string{"function"},
	)
	functionAnnualCostMetric = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "cost_profiler_function_annual_cost_rub",
			Help: "Projected annual cost in RUB per function",
		},
		[]string{"function"},
	)
)
...
func init() {
	prometheus.MustRegister(functionCallsMetric, functionCyclesMetric, functionAnnualCostMetric)
}
...
// ---------- perf_event_open для CPU cycles ----------
func openPerfEvent(pid, tid int) (int, error) {
	attr := syscall.PerfEventAttr{
		Type:        syscall.PERF_TYPE_HARDWARE,
		Config:      syscall.PERF_COUNT_HW_CPU_CYCLES,
		Size:        uint32(unsafe.Sizeof(syscall.PerfEventAttr{})),
		Sample_type: 0,
		Sample:      0,
		Bits:        0,
	}
	fd, err := syscall.PerfEventOpen(&attr, pid, tid, -1, 0)
	if err != nil {
		return -1, err
	}
	return fd, nil
...
// watchPerfMap следит за изменениями файла perf-map
...
func printTable(metrics []FunctionMetric, config CostConfig) {
	...
	for _, m := range metrics {
		if m.CallCount == 0 {
			table.Rich([]string{
				color.New(color.FgHiBlack).Sprint(m.Name),
				"-",
				"-",
				"-",
				"(inlined, нет данных)",
...
func main() {
	...
	flag.StringVar(&namespace, "namespace", "default", "Namespace Pod")
	...
	flag.StringVar(&exportPGO, "export-pgo", "", "Экспортировать профиль в формате pprof")
	flag.StringVar(&lang, "lang", "go", "Язык приложения: go, java")
	flag.BoolVar(&javaSignal, "java-signal", false, "Отправить SIGUSR2 процессу Java для генерации perf-map")
	flag.StringVar(&webPort, "web", "", "Запустить веб-интерфейс (и Prometheus /metrics) на порту (например, :8080)")
	flag.Parse()
	...
	case "go":
		...
		cyan.Printf("🔍 Обнаружено Go-функций (включая inlined): %d\n", len(symbols))
	case "java":
		if javaSignal {
			if err := proc.Signal(syscall.SIGUSR2); err != nil {
				yellow.Printf("⚠️ Не удалось отправить SIGUSR2: %v\n", err)
			} else {
				fmt.Println("📡 Сигнал SIGUSR2 отправлен Java-процессу. Ожидаем генерацию /tmp/perf-*.map...")
	...
	if len(symbols) == 0 {
		yellow.Println("⚠️ Функции не найдены. Для Go: соберите с -ldflags=\"-s=false\". Для Java: проверьте наличие /tmp/perf-<pid>.map")
		os.Exit(1)
	}
	...
	zeroKey := uint32(0)
	samplingVal := uint32(sampling)
	if err := objs.SamplingRate.Put(&zeroKey, &samplingVal); err != nil {
		log.Printf("⚠️ Не удалось установить sampling_rate в eBPF: %v", err)
	}
	...
	// Настройка perf_event для CPU cycles
	tidMap := make(map[uint64]bool)
	...
	if len(tidMap) == 0 {
		tidMap[uint64(pid)<<32|uint64(pid)] = true
	}
	for key := range tidMap {
		tid := int(key & 0xffffffff)
		fd, err := openPerfEvent(pid, tid)
		if err != nil {
			yellow.Printf("⚠️ Не удалось открыть perf_event для tid %d: %v", tid, err)
			continue
		}
		if err := objs.PerfFds.Put(&key, &fd); err != nil {
			yellow.Printf("⚠️ Не удалось записать fd в eBPF для tid %d: %v", tid, err)
			syscall.Close(fd)
		} else {
			green.Printf("  📊 perf_event для tid %d (fd=%d)", tid, fd)
		}
	}
	defer func() {
		var key, fd uint64
		iter := objs.PerfFds.Iterate()
		for iter.Next(&key, &fd) {
	...
	// Запуск веб-сервера (Prometheus + JSON + HTML)
	if webPort != "" {
		http.Handle("/metrics", promhttp.Handler())           // Prometheus формат
		http.HandleFunc("/api/metrics", metricsHandler)       // JSON для таблицы
		http.HandleFunc("/", indexHandler)                   // HTML дашборд
		go func() {
			fmt.Printf("🌐 Веб-интерфейс запущен на http://localhost%s\n", webPort)
			fmt.Printf("   - Prometheus метрики: http://localhost%s/metrics\n", webPort)
			fmt.Printf("   - JSON API: http://localhost%s/api/metrics\n", webPort)
	...
	for _, m := range metricList {
		...
		functionCallsMetric.WithLabelValues(m.Name).Add(float64(m.CallCount))
		functionCyclesMetric.WithLabelValues(m.Name).Add(float64(m.TotalNS))
		functionAnnualCostMetric.WithLabelValues(m.Name).Set(m.AnnualCost)
	}

	metricsMutex.Lock()
	lastMetrics = metricList
	metricsMutex.Unlock()
	...
	if hasInlined {
		fmt.Println()
		yellow.Println("💡 Обнаружены inlined функции. Для более точного профилирования добавьте //go:noinline перед соответствующими функциями в Go.")
	}
	...
	if webPort != "" {
		fmt.Println("🖥️ Веб-интерфейс продолжает работу. Нажмите Ctrl+C для завершения.")
		<-sigChan
		fmt.Println("\nЗавершение работы.")
	}
}
