...
- 🎯 **Автоопределение языка** — Go/Java определяются автоматически
- 💾 **Долгосрочное хранение метрик** — ClickHouse / Prometheus Remote Write
- 🤖 **AI-рекомендации** — автоматический анализ профиля с предложениями оптимизации
- 🚨 **SLO мониторинг и алерты** — Slack, PagerDuty, Opsgenie, Telegram
...
sudo cost-profiler --pid $(pgrep java) --cost 4.5 --duration 30

# Профилирование Pod’а в Kubernetes
sudo cost-profiler --pod my-service-7fbd --namespace prod --binary /app/service
...
Cost Profiler поставляется с готовым дашбордом для Grafana.
...
Открой `http://localhost:3000` (логин: admin, пароль: admin) и выбери **Cost Profiler Dashboard**.

Дашборд показывает:
- 💰 Общую годовую стоимость в рублях
- 📞 Общее количество вызовов функций
- ⚡ Среднее количество CPU циклов на вызов
- 🎯 Потенциальную экономию от оптимизации
- 🏆 Топ-10 самых дорогих функций
- 📈 Графики вызовов во времени
- 🌐 Распределение затрат по языкам
- ⚠️ Превышение бюджета

### Импорт вручную

Если хочешь импортировать дашборд в существующую Grafana:

1. Открой Grafana → **Dashboards** → **Import**
2. Загрузи файл `deploy/grafana/dashboards/cost-profiler.json`
3. Выбери DataSource `Prometheus`
4. Нажми **Import**
...
```bash
sudo cost-profiler --pid $(pgrep java) --cost 4.5
```
...
```bash
sudo cost-profiler --pod my-pod --namespace default --binary /app/service
```
...
sudo cost-profiler --pid $(pgrep java) --java-signal --web :8080

# Kubernetes pod + фильтр функций
sudo cost-profiler --pod my-app-7fbd --namespace prod --binary /app/service --filter "main."
...
- [x] **Автоопределение языка (--lang auto)**
- [x] **Сохранение истории в ClickHouse / Prometheus**
- [x] **AI-рекомендации по оптимизации**
- [x] **SLO мониторинг и алерты**
- [x] **Grafana Dashboard**
