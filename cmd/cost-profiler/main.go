...
// parsePerfMap reads /tmp/perf-<pid>.map and returns address->name
...
// watchPerfMap monitors /tmp/perf-<pid>.map for changes and attaches uprobes for new methods
...
// findSymbolsForGo returns functions from Go binary using DWARF (inlined detection)
func findSymbolsForGo(binaryPath string, filter string) ([]Symbol, error) {
	return getAllSymbolsWithDWARF(binaryPath, filter)
}

// findSymbolsForJava returns JIT methods from perf-map
...
// openPerfEvent opens a perf_event for CPU cycles for given PID and TID
...
// getAllThreads returns a map of pid_tgid (uint64) for each thread of a given process
func getAllThreads(pid int) map[uint64]bool {
...
func main() {
	...
	flag.StringVar(&podName, "pod", "", "Имя Pod (вместо --pid)")
	flag.StringVar(&namespace, "namespace", "default", "Namespace Pod")
	flag.StringVar(&containerName, "container", "", "Имя контейнера в Pod")
	flag.StringVar(&binaryPath, "binary", "", "Путь к бинарному файлу (для Go)")
	...
	flag.BoolVar(&javaForceMap, "java-force-map", false, "Отправить SIGUSR2 процессу Java для генерации perf-map")
	flag.Parse()
	...
	case "java":
		// Для Java: если нужно принудительно сгенерировать perf-map
		if javaForceMap {
			if err := proc.Signal(syscall.SIGUSR2); err != nil {
				yellow.Printf("⚠️ Не удалось отправить SIGUSR2 Java процессу: %v", err)
			} else {
				green.Println("✅ Отправлен SIGUSR2 для генерации perf-map")
	...
	if len(symbols) == 0 {
		yellow.Println("⚠️ Функции не найдены. Для Go: соберите с -ldflags=\"-s=false\". Для Java: проверьте /tmp/perf-<pid>.map")
		os.Exit(1)
	}
	...
	// Установка семплинга в eBPF
	samplingKey := uint32(0)
	samplingVal := uint32(sampling)
	if err := objs.SamplingRate.Put(&samplingKey, &samplingVal); err != nil {
		log.Printf("⚠️ Не удалось установить sampling_rate: %v", err)
	}

	// Открытие perf_event для всех потоков процесса и запись fd в eBPF map
	threads := getAllThreads(pid)
	var perfFds []int
